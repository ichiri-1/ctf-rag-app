...
INDEX_PATH = Path(settings.app_data_dir) / "index.json"
...
def build_context(results: list[dict[str, Any]]) -> str:
    ...
    for item in results:
        lines.append(
            f"[{item['chunk_id']}] doc_id={item['doc_id']} score={item['score']:.4f}\n"
            f"{item['text']}"
        )
    return "\n\n".join(lines)
...
def generate_answer(question: str, results: list[dict[str, Any]]) -> str:
    ...
    prompt = f"""### 指示
あなたは日本語で答える簡単な研究補助AIです。
与えられたコンテキストだけを根拠にして、質問に簡潔に答えてください。
情報が足りない場合は「分かりません」と答えてください。
可能なら参照した chunk_id を [] で示してください。
...
def ingest(payload: IngestRequest) -> dict[str, int]:
    ...
    return ingest_documents(INDEX_PATH, docs)


@app.post("/query")
def query(payload: QueryRequest) -> dict[str, Any]:
    top_k = payload.top_k or settings.retrieval_top_k
    results = retrieve(INDEX_PATH, payload.question, top_k)
